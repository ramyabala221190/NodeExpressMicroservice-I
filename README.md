...
# Deployment strategy on single server without swarm
...
We need to have 3 instances of the cart-node. Instead of providing 3 duplicates of the
same service within the compose file, we just define 1 service with the name:cart-node.
...
"--scale cart-node=3" will create 3 instances of the cart-node

Below is an example for local run:

```
 "docker-local-dev-up": "cross-env TARGETENV=dev docker compose --env-file docker/environments/local.env -p gateway-dev -f docker/docker-compose.local.yml -f docker/docker-compose.dev.override.yml up -d --remove-orphans --no-build --scale cart-node=3",
```
...
So go for cross-env which works irrespective of the OS.
...
For cart-node , we have used expose instead of ports field.
...
Container ports can remain the same for the 3 instances of cart-node service
...
In docker, check the below variables set in docker compose. The paths are different
